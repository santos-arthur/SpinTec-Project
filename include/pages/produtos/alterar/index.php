    ...
    <form class='container containerForm col-md-6 col-sm-12'>
        ...
        <div class='row'>
            ...
            <div class='col-sm-12 col-md-9 center insideMargin10px'>
                <div>
                    <input type="radio" id="ativoSim" name="inputAtivo" value="1" default>
...
$(document).ready(function(){
    $.ajax({
        type: "POST",
        url: '<?=implode('/', array(publicLink,'api','produtos','dados'))?>',
        data: {
            'id' : '<?=$_REQUEST['id']?>'
        },
        success: function(data){
            if(data.status == true){
                ...
                $('#inputCodigoBarras').val(data.dados.codigoBarras);
                $('#inputValor').val(data.dados.valorUnitario);
                ...
                Swal.fire({
                    title: data.mensagem,
                    text: 'Voltar a tela de produtos?',
                    icon: 'error',
...
function salvar(){
    ...
    $.ajax({
        type: "POST",
        url: '<?=implode('/', array(publicLink,'api','produtos','alterar'))?>',
        data: {
            'id'            : '<?=$_REQUEST['id']?>',
            'ativo'         : ativo,
            'nome'          : nome,
